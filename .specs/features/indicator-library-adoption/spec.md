...
| Band / cloud / fill shapes (Ichimoku, Bollinger fill) | `SeriesSpec.shape` is `'line' \| 'histogram' \| 'area'` and `laneDraft` hard-codes `'line'`. Widening it is its own ticket, already recorded as an open issue in STATE.md |
| Fixing label-as-identity (`ChartWorkspace.tsx:298` persists `entry.label`) | A real defect with its own blast radius — translating a label would drop the persisted studies. It is a prerequisite risk this spec works around, not a requirement it fixes |
...
| State-transition integrity | PARAM-01, PARAM-02 — editing a value must not change a study's identity, its lane, or its position in the list |
...
1. WHEN a tab carrying per-study parameter values is serialised and parsed back THEN the workspace SHALL restore each study with the same values it was saved with  <!-- event-driven -->
...
1. WHEN the demo loads THEN it SHALL offer a curated catalogue drawn from the library, excluding entries that return no plot  <!-- event-driven -->
...
- IF two catalogue entries share a label THEN the second SHALL be reported by a host-side check, because `laneOrder` deduplicates by exact string and the second toggle would otherwise do nothing forever
- WHEN the active study count exceeds the lane count THEN the workspace SHALL continue to cut from the end, unchanged by this feature
